...
df_descriptive <- df[,descriptive_variables] # since the variables have varying distribution, we first must normalize, if we want to visualize them in one histogram
df_descriptive %>%
  # Reshape
  gather(key = indicator, value = val) %>%
  # Basic chart
  ggplot(aes(x =val)) +
  geom_histogram(colour = "darkgreen", fill = "gray",bins = 30) +
  scale_x_continuous(trans='log10',labels = NULL,name = NULL) +           #Just a starting point, we need to further polish that. I had to omit mode and loudness as they screwed up the histogram so much
